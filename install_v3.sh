...
# Programmiersprachen hier angeben
# Ordner erstllung
proglang=(
  "lua"
  "clang"
...
for prog in "${proglang[@]}"; do
  echo -e "\n \n \n"
  echo -e "\n ------------------------------------"
  echo -e "erstellt $prog"
  notify-send "$prog wird erstellt"
  mkdir -p /home/$user_name/$prog/ >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
  mkdir -p /home/$user_name/$prog/doku >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
  mkdir -p /home/$user_name/$prog/prog >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
  mkdir -p /home/$user_name/$prog/git >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
  mkdir -p /home/$user_name/Documents/latex/prog/$prog/ >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
done

mkdir -p /home/$user_name/log >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
...
mkdir -p /home/$user_name/git1/ >>"$LOG_FILE" 2>>"$ERROR_LOG_FILE"
